...
(function () {
  ...
  function mk(id, cfg) {
    ...
    cfg.options = Object.assign({ responsive: true, maintainAspectRatio: false, animation: { duration: 200 },
      plugins: { legend: { labels: { color: tick, font: { size: 11 }, boxWidth: 12, boxHeight: 12, usePointStyle: true } }, tooltip: { backgroundColor: PAL.darkred, titleColor: '#fff', bodyColor: '#fff', padding: 10, cornerRadius: 8 }, datalabels: { display: true, color: '#FFFFFF', font: { size: 10, weight: 'bold' }, anchor: 'center', align: 'center', formatter: dlFmt || ((v) => v == null ? '' : v) } },
  ...
  function renderVrio() {
    ...
    mk('cVrio', { type: 'bar', data: { labels: ranked.map(d => d.name), datasets: [{ data: ranked.map(d => +d.s.toFixed(1)), backgroundColor: ranked.map(d => d.s >= 80 ? PAL.green : d.s >= 60 ? PAL.amber : '#64748B'), borderRadius: 4 }] }, dlFmt: v => v.toFixed(1), options: { indexAxis: 'y', plugins: { legend: { display: false } }, scales: { x: { min: 0, max: 100, ticks: { callback: v => v } } } } });
  }
