...
(function () {
  ...
  function renderYtd() {
    ...
    mk('cMom', { type: 'bar', data: { labels, datasets: [{ data: mom, backgroundColor: mom.map(m => m == null ? PAL.gray : m >= 0 ? PAL.green : PAL.red), borderRadius: 3 }] }, options: { plugins: { legend: { display: false } }, scales: { y: { ticks: { callback: v => v + '%' } } } } });
  }
