...
(function () {
  ...
  function mk(id, cfg) {
    ...
    cfg.options = Object.assign({ responsive: true, maintainAspectRatio: false, animation: { duration: 200 },
      plugins: { legend: { labels: { color: tick, font: { size: 11 }, boxWidth: 12, boxHeight: 12, usePointStyle: true } }, tooltip: { backgroundColor: PAL.darkred, titleColor: '#fff', bodyColor: '#fff', padding: 10, cornerRadius: 8 } },
  ...
  function renderYtd() {
    ...
    $('#content').querySelector('[data-view-panel="ytd"]').innerHTML =
      ...
      '</div>' +
      '<div class="card"><div class="card-title">Monthly Target vs Achievement</div><div class="chart-box lg"><canvas id="cYtd"></canvas></div></div>' +
      '<div class="card"><div class="card-title">Monthly Detail</div>' +
      tbl(['Month', 'Target', 'Actual', 'Achievement %', 'Gap'], labels.map((l, i) => [l, 'BDT ' + FMT.money(series[i].t), 'BDT ' + FMT.money(series[i].a), series[i].t > 0 ? (series[i].a / series[i].t * 100).toFixed(1) + '%' : '—', 'BDT ' + FMT.money(series[i].t - series[i].a)]), [1, 2, 3, 4]) + '</div>';
    mk('cYtd', { type: 'bar', data: { labels, datasets: [
      { label: 'Target', data: labels.map((_, i) => series[i].t), backgroundColor: 'rgba(169,180,192,.4)', borderRadius: 3 },
      { label: 'Actual', data: labels.map((_, i) => series[i].a), backgroundColor: PAL.red, borderRadius: 3 }
    ] }, options: { scales: { y: { ticks: { callback: v => 'BDT ' + FMT.money(v) } } } } });
  }
